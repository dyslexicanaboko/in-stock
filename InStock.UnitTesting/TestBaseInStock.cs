...
namespace InStock.UnitTesting
{
    ...
    : TestBase
  {
    ...
    protected IAppConfiguration GetConfiguration()
    {
      var config = A.Fake<IAppConfiguration>();

      A.CallTo(() => config.QuoteCacheWindowMinutes).Returns(60);

      return config;
    }

    protected StockEntity GetSomeStock() => new StockEntity
    {
      Symbol = SomeSymbol,
      Name = "FLoriDUHH",
      Notes = "Florida sucks",
    ...
    protected QuoteEntity GetSomeQuote() => new QuoteEntity
    {
      QuoteId = 1,
      StockId = SomeStockId,
      Date = TodayUtc,
    ...
    protected List<PositionEntity> GetSomePositions(int count = 3)
    {
      ...
      for (var i = 1; i <= count; i++) lst.Add(GetSomePosition(i));
    ...
    protected List<TradeEntity> GetSomeTrades(int count = 3)
    {
      ...
      for (var i = 1; i <= count; i++) lst.Add(GetSomeTrade(i));
    ...
    protected IList<EarningsEntity> GetMultipleEarnings(int count)
    {
      ...
      for (var i = 1; i <= count; i++) lst.Add(GetSomeEarnings(i));

      return lst;
    }

    protected EarningsEntity GetSomeEarnings(int increment = 1) => new EarningsEntity
    {
    ...
    private static void AssertThrowsNotFoundException(TestDelegate testMethod, int errorCode)
    {
    ...
    private static void AssertIsNotFoundException(Exception ex, int errorCode)
    {
